...
namespace InternetShop.Web.Controllers
{
    public class OrderController : Controller
    {
        ...
        [HttpPost]
        public IActionResult ConfirmCellPhone(int id, string cellPhone, int confirmationCode)
        {
        ...
        public IActionResult StartDelivery(string serviceName)
        {
            ...
            var returnUri = GetReturnUti(nameof(NextDelivery));
        ...
        private Uri GetReturnUti(string action)
        {
        ...
        public IActionResult NextDelivery(string serviceName, int step, Dictionary<string, string> values)
        {
            ...
            var paymentsMethods = paymentServices.ToDictionary(service => service.Name,
                                                               service => service.Title);

            return View("PaymentMethod", paymentsMethods);
        }
        ...
        public IActionResult StartPayment(string serviceName)
        {
            ...
            var returnUri = GetReturnUti(nameof(NextPayment));
            var redirectUri = webContractorService.StartSession(form.Parameters, returnUri);
